...
with open ('addtionalNewID.txt') as f:
    lines = f.readlines()
for line in lines:
...
with open ('allDomainByAdditionID.txt', 'w') as f:
    for ID in IDs:
        f.write(ID + ':')
...
with open ('domainByAdditionIDCount.txt', 'w') as f:
    for ID in IDs:
        f.write(ID + ':' + str(len(allDomains[ID])) + '\n')

with open ('domainsByAdditionID.txt', 'w') as f:
